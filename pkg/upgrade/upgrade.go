...
func Upgrade(instance, namespace, sourceVersion, targetVersion string, force, skipChecks bool, kubeconfig string) error {
	...
	if utils.IsPatchUpgrade(sourceVer, targetVer) {
		return PatchUpgrade(client, instance, namespace, targetVersion)
	} else if utils.IsMinorUpgrade(sourceVer, targetVer) {
